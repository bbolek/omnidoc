...
### Fixed
- Linux: app launched to a completely black window on many systems (notably machines with NVIDIA proprietary drivers and some Wayland compositors). WebKit2GTK 2.42+ enables a DMABUF-backed compositor by default that never produces a frame on these configurations, so WRY's surface existed but stayed black. The Rust entry point now sets `WEBKIT_DISABLE_DMABUF_RENDERER=1` before Tauri initializes (unless the user has set it explicitly), falling back to the software renderer that draws reliably.
